...
async function main(){
  ...
  if(!usdtAddr || usdtAddr===""){ const Mock = await (await ethers.getContractFactory("MockUSDT")).deploy(USDT_DEC); await Mock.waitForDeployment(); usdtAddr = Mock.target as string; console.log("MockUSDT:", usdtAddr); }
  ...
  console.log("USDT:", usdtAddr); console.log("NBL:", NBL.target); console.log("REG:", REG.target); console.log("TREE:", TREE.target); console.log("POOL:", POOL.target); console.log("SALE:", SALE.target);
}
